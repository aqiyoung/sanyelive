...
class _PlayerPageState extends ConsumerState<PlayerPage> {
  ...
  // 优化 (8/5): 频道查找索引 — 全量约 2500+ 条, 每次 build/data 回调线性扫
  // 太慢.  按 list 实例缓存 Map<String,Channel>, list 不变则 O(1) 命中.
  Map<String, Channel>? _channelIndex;
  List<Channel>? _channelIndexSource;
  ...
  Channel? _findChannel(List<Channel> all, String id) {
    // list 实例不变则复用索引, 否则重建一次 (O(n)), 之后查找 O(1).
    if (!identical(all, _channelIndexSource)) {
      _channelIndexSource = all;
      _channelIndex = {for (final c in all) c.id: c};
    }
    return _channelIndex![id];
  }
