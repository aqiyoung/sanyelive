...
/// 计算最近一次 Beijing（UTC+8）指定墙钟时间对应的绝对时刻。
/// 之前直接用 DateTime.now() 的本地时区算 03:00，设备不在 UTC+8 时
/// "北京时间 03:00" 调度会错。这里统一按 UTC+8 偏移、用 DateTime.utc
/// 构造避免受设备本地时区干扰。
DateTime _nextBeijingRefresh(int hour, [int minute = 0]) {
  const beijing = Duration(hours: 8);
  final nowUtc = DateTime.now().toUtc();
  final beijingNow = nowUtc.add(beijing); // 北京墙钟对应的绝对偏移表示
  // Beijing wall = UTC + 8h  =>  绝对时刻 = DateTime.utc(北京日期, h, m) - 8h
  DateTime atBeijing(int addDays) => DateTime.utc(
        beijingNow.year,
        beijingNow.month,
        beijingNow.day,
        hour,
        minute,
      ).subtract(beijing).add(Duration(days: addDays));
  var target = atBeijing(0);
  if (!target.isAfter(nowUtc)) target = atBeijing(1);
  return target;
}

void _scheduleNextChannelsRefresh(ProviderContainer container) {
  final now = DateTime.now();
  final next = _nextBeijingRefresh(3); // Beijing 03:00
  final delay = next.difference(now);
  final bj = next.toUtc().add(const Duration(hours: 8));
  debugPrint(
      'ChannelsRefresh: 下次刷新 ${delay.inHours}h ${delay.inMinutes.remainder(60)}m 后 (Beijing ${bj.year}-${bj.month}-${bj.day} ${bj.hour}:${bj.minute})');
...
void _scheduleNextSourcesRefresh(ProviderContainer container) {
  ...
  final bj = next.toUtc().add(const Duration(hours: 8));
  debugPrint(
      'SourcesRefresh: 下次刷新 ${delay.inHours}h ${delay.inMinutes.remainder(60)}m 后 (Beijing ${bj.year}-${bj.month}-${bj.day} ${bj.hour}:${bj.minute})');
